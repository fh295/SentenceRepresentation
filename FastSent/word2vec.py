...
try:
    ...
    def train_sentence_cbow(model, sentences, alpha, work=None, neu1=None):
...
class Word2Vec(utils.SaveLoad):
    ...
    def __init__(
            ...
            sg=1, hs=1, negative=0, cbow_mean=0, hashfxn=hash, iter=1, null_word=0, autoencode=0):
        """
    ...
    def _do_train_job(self, job, alpha, inits):
        ...
        for i in range(0, len(job)-2):
            ...
            if self.autoencode:
                raw_tally += len(job[i]) + len(job[i+1]) + len(job[i+2])
            else:
                raw_tally += len(job[i]) + len(job[i+2])
