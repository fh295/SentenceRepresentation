...
cdef void fast_sentence_cbow_hs(
    ...
    REAL_t *work, int len_sentence, int cbow_mean, REAL_t *word_locks) nogil:
...
#
#
#     # neu1 is storing the sum of cbow
#     memset(neu1, 0, size * cython.sizeof(REAL_t))
#     count = <REAL_t>0.0
#
#     # performing a saxpy to sum the representation
#     for m in range(j, k):
#         if m == i:
#             continue
#         else:
...
#             continue
#         else:
#             # update the representations at position indexes[m] in syn0 with the gradient
#             our_saxpy(&size, &word_locks[indexes[m]], work, &ONE, &syn0[indexes[m] * size], &ONE)
...
def train_sentence_cbow(model, sentences, alpha, _work, _neu1):
    ...
    with nogil:
        for i in range(context_len):
            # if hs:
            fast_sentence_cbow_hs(points[i], codes[i], codelens[i], neu1, syn0, syn1, size, indexes, _alpha, work, sentence_len, cbow_mean, word_locks)
            # if negative:
            #     next_random = fast_sentence_cbow_neg(negative, cum_table,
            #     cum_table_len, codelens, neu1, syn0, syn1neg, size, indexes,
            #     _alpha, work, sentence_len, cbow_mean, next_random, word_locks)
...
#     neu1 = <REAL_t *>np.PyArray_DATA(_neu1)
#
#     # getting the vocabulary lookup table
#     vlookup = model.vocab
#
#     current_sentence = sentences[1]
#     context_sentence = sentences[0] + sentences[2]
#
#     i = 0
#     for token in sentence:
#         # getting word id in the dictionary
#         word = vlookup[token] if token in vlookup else None
#         # if not in the dictionary, drop it
#         if word is None:
#             continue  # leaving i unchanged/shortening sentence
